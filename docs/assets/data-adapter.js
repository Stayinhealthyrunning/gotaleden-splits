(function(){
  ...
  function genericClassMeta(value){const record=typeof value==='object'&&value?value:null,name=normalizeRelayClassName(record?.class_name??value),competitionType=record?.class_competition_type||'competitive',ranked=competitionType==='non_competitive'?false:record?.class_is_ranked!==false;return{id:`other-${name.toLocaleLowerCase('sv').replace(/[^a-z0-9åäö]+/g,'-').replace(/^-|-$/g,'')||'unknown'}`,family:'other',shortLabel:name||'Klass saknas',fullLabel:name||'Klass saknas',ranked,competitionType,color:'#66756f',softColor:'#edf1ef'}}

  function create(data,courseInput,elevationInput){
    const races=new Map(),raceCatalog=new Map(Object.entries(data.race_catalog||{})),records=new Map(),splitsByResult=new Map(),teams=new Map(),profileCache=new Map(),referenceCache=new Map(),journeyReferenceCache=new Map(),courseAssets=new Map(),classDefinitions=new Map(),classOrder=new Map();
    ...
    for(const [raceKey,source] of Object.entries(data.races||{})){
      ...
      const participant=source.participant||{entity:'person',singular:'deltagare',plural:'deltagare',profile_label:'DELTAGARANALYS',possessive:'Deltagarens'},competition=source.competition||{format:source.type||'race',team_structure:{kind:'none',member_assignment:'unknown'}},capabilities=source.capabilities||{},presentation=source.presentation||{},uiLabels=source.ui_labels||{},goalPace=source.goal_pace||null;
      for(const [index,item] of (source.class_metadata?.classes||[]).entries()){const name=normalizeRelayClassName(item.source_name);classDefinitions.set(`${raceKey}|${name}`,{id:item.id,family:item.family||item.id,shortLabel:item.short_label||name,fullLabel:name,ranked:item.ranked!==false,competitionType:item.competition_type||'competitive',comparisonFocus:item.comparison_focus===true,color:item.color||'#66756f',softColor:item.soft_color||'#edf1ef'});classOrder.set(`${raceKey}|${name}`,index)}
      const race={
        key:raceKey,
        ...
        type:source.type,
        event:data.meta?.event||{},participant,participantEntity:participant.entity,isTeam:participant.entity==='team',competition,competitionFormat:competition.format,teamStructure:competition.team_structure||{kind:'none',member_assignment:'unknown'},capabilities,uiLabels,presentation,goalPace,classScheme:source.class_scheme||null,
    ...
    function relayClassMeta(value){const item=typeof value==='string'?null:value,recordValue=item?.raceKey?item:null,name=normalizeRelayClassName(recordValue?.class_name??value),raceKey=recordValue?.raceKey,configured=classDefinitions.get(`${raceKey}|${name}`)||[...classDefinitions.entries()].find(([key])=>key.endsWith(`|${name}`))?.[1],base=configured||genericClassMeta(recordValue||name),competitionType=recordValue?.class_competition_type||base.competitionType,ranked=competitionType==='non_competitive'?false:recordValue?.class_is_ranked===undefined||recordValue?.class_is_ranked===null?base.ranked:recordValue.class_is_ranked===true||recordValue.class_is_ranked===1||recordValue.class_is_ranked==='true';return{...base,fullLabel:name||base.fullLabel,ranked,competitionType}}
    function checkpointName(raceValue,checkpoint){return checkpoint?.key===raceValue?.analysisCheckpoints?.at(-1)?.key&&raceValue.presentation.finish_label||checkpoint?.name||''}
    ...
    function team(value){const item=typeof value==='string'?record(value):value;return item?.isTeam?teams.get(`${item.raceKey}:${item.bib}`):null}
    ...
    function profile(value){
      const item=typeof value==='string'?record(value):value;if(profileCache.has(item.id))return profileCache.get(item.id);const raceValue=race(item.raceKey),known=item.status==='DNS'?[]:resultSplits(item),anchors=[{checkpoint:raceValue.checkpoints[0].key,name:raceValue.checkpoints[0].name,elapsedSeconds:0,distance:raceValue.startDistanceKm,placeOverall:null,placeClass:null,placeGender:null,kind:'start'}];
    ...
    function referenceProfiles(value){
      const item=typeof value==='string'?record(value):value,raceValue=race(item.raceKey),cacheKey=`${raceValue.key}|${item.class_name||'-'}|${raceValue.isTeam?'team':item.sex||'-'}`;if(referenceCache.has(cacheKey))return referenceCache.get(cacheKey);const complete=completeProfiles(raceValue),field=cohortReference('field',raceValue.isTeam?(raceValue.uiLabels.field||'Hela deltagarfältet'):'Hela fältet','#66756f',complete,raceValue),classProfiles=complete.filter(candidate=>candidate.record.class_name===item.class_name),classReference=cohortReference('class',raceValue.isTeam?(raceValue.uiLabels.class_reference||'Min klass'):'Min klass','#138a78',classProfiles,raceValue),sexProfiles=raceValue.isTeam?[]:complete.filter(candidate=>candidate.record.sex===item.sex),sexReference=raceValue.isTeam?{id:'sex',label:'Mitt kön',color:item.sex==='F'?'#db2777':'#2563eb',count:0,available:false,message:`Könsreferens används inte för ${raceValue.participant.plural}`} :cohortReference('sex','Mitt kön',item.sex==='F'?'#db2777':'#2563eb',sexProfiles,raceValue);const same=classReference.available&&sexReference.available&&classReference.cohortIds.join('|')===sexReference.cohortIds.join('|');if(same){classReference.coincidesWith='sex';sexReference.coincidesWith='class'}const result={field,class:classReference,sex:sexReference,completeCount:complete.length,minimumSize:MIN_REFERENCE_SIZE};referenceCache.set(cacheKey,result);return result;
    }
    ...
    function segmentDistributionProfile(raceValue,profiles){
      ...
      return boundaries.slice(1).map((to,index)=>{const from=boundaries[index],samples=cohort.map(candidate=>candidate.segments[index]).filter(segment=>segment?.from.checkpoint===from.key&&segment?.to.checkpoint===to.key&&finite(segment.paceSecondsKm)&&Number(segment.paceSecondsKm)>0),pace=distributionSummary(samples.map(segment=>segment.paceSecondsKm)),toName=index===boundaries.length-2&&raceValue.presentation.finish_label||to.name;return{id:`${from.key}--${to.key}`,index,from:from.key,to:to.key,name:`${from.name}–${toName}`,fromDistance:Number(from.route_distance_km),toDistance:Number(to.route_distance_km),distanceKm:Number(to.route_distance_km)-Number(from.route_distance_km),pace,n:pace.n,samples}});
    }
    ...
    function journeyReferences(value){
      const item=typeof value==='string'?record(value):value,raceValue=race(item.raceKey),cacheKey=`${raceValue.key}|${item.class_name||'-'}|${raceValue.isTeam?'team':item.sex||'-'}`;if(journeyReferenceCache.has(cacheKey))return journeyReferenceCache.get(cacheKey);const complete=journeyCompleteProfiles(raceValue),field=journeyReference('field',raceValue.isTeam?(raceValue.uiLabels.field||'Hela deltagarfältet'):'Hela fältet','#0b6671',complete,raceValue),classProfiles=complete.filter(candidate=>candidate.record.class_name===item.class_name),classReference=journeyReference('class',raceValue.isTeam?(raceValue.uiLabels.class_reference||'Min klass'):'Min klass','#138a78',classProfiles,raceValue),references=[field,classReference];if(!raceValue.isTeam)references.push(journeyReference('sex','Mitt kön',item.sex==='F'?'#db2777':'#2563eb',complete.filter(candidate=>candidate.record.sex===item.sex),raceValue));const result={race:raceValue,references,field,class:classReference,sex:references.find(reference=>reference.id==='sex')||null,completeCount:complete.length,minimumSize:MIN_REFERENCE_SIZE,analyticSegmentCount:raceValue.analysisCheckpoints.length-1};journeyReferenceCache.set(cacheKey,result);return result;
    }
    ...
    function journeyAnalysis(value,referenceId='field'){
      const item=typeof value==='string'?record(value):value,raceValue=race(item.raceKey),profileValue=profile(item),referenceSet=journeyReferences(item),reference=referenceSet.references.find(candidate=>candidate.id===referenceId&&candidate.available)||referenceSet.references.find(candidate=>candidate.available)||referenceSet.references[0],checkpoints=raceValue.analysisCheckpoints.slice(1),relayMeta=relayClassMeta(item),finishKey=raceValue.analysisCheckpoints.at(-1)?.key,gapPoints=checkpoints.map((checkpoint,index)=>{const anchor=checkpoint.key===finishKey&&!statusFinished(item)?null:profileValue.analysisAnchors.find(candidate=>candidate.checkpoint===checkpoint.key),medianElapsed=reference?.checkpoints[index]?.medianElapsed,gapSeconds=anchor?journeyGap(medianElapsed,anchor.elapsedSeconds):null;return{index,checkpoint:checkpoint.key,name:checkpointName(raceValue,checkpoint),distance:Number(checkpoint.route_distance_km),elapsedSeconds:anchor?.elapsedSeconds??null,referenceMedianElapsed:medianElapsed??null,gapSeconds,sourceType:anchor?'official':null,referenceType:'derived-median'}}),placementPoints=checkpoints.map((checkpoint,index)=>{const finish=checkpoint.key===finishKey&&statusFinished(item),anchor=checkpoint.key===finishKey&&!finish?null:profileValue.analysisAnchors.find(candidate=>candidate.checkpoint===checkpoint.key),overall=finish&&finite(item.overall_place)?Number(item.overall_place):anchor?.placeOverall??null,classPlace=finish&&finite(item.class_place)?Number(item.class_place):anchor?.placeClass??null,gender=finish&&finite(item.gender_place)?Number(item.gender_place):anchor?.placeGender??null;return{index,checkpoint:checkpoint.key,name:checkpointName(raceValue,checkpoint),distance:Number(checkpoint.route_distance_km),overall:anchor||finish?overall:null,class:relayMeta.ranked&&(anchor||finish)?classPlace:null,sex:!raceValue.isTeam&&(anchor||finish)?gender:null,sourceType:anchor||finish?'official':null}}),pacing=checkpoints.map((checkpoint,index)=>{const expectedFrom=raceValue.analysisCheckpoints[index],segment=profileValue.segments.find(candidate=>candidate.from.checkpoint===expectedFrom.key&&candidate.to.checkpoint===checkpoint.key),ownPace=segment?.paceSecondsKm??null,referenceMedianPace=reference?.checkpoints[index]?.medianPace??null,relativePerformance=journeyRelativePerformance(referenceMedianPace,ownPace),category=journeyPacingCategory(relativePerformance);return{index,checkpoint:checkpoint.key,name:`${checkpointName(raceValue,expectedFrom)}–${checkpointName(raceValue,checkpoint)}`,fromDistance:Number(expectedFrom.route_distance_km),toDistance:Number(checkpoint.route_distance_km),midpointDistance:(Number(expectedFrom.route_distance_km)+Number(checkpoint.route_distance_km))/2,ownPace,referenceMedianPace,relativePerformance,...category,sourceType:segment?'official':null,referenceType:'derived-median'}});return{record:item,race:raceValue,profile:profileValue,references:referenceSet.references,reference,minimumSize:MIN_REFERENCE_SIZE,gapPoints,placementPoints,pacing,placementSeries:{overall:{id:'overall',label:'Total',available:placementPoints.some(point=>finite(point.overall)),values:placementPoints.map(point=>point.overall)},class:{id:'class',label:raceValue.isTeam?(raceValue.uiLabels.class||'Klass'):'Klass',available:relayMeta.ranked&&placementPoints.some(point=>finite(point.class)),message:relayMeta.ranked?'Placering saknas':'Ej tävling – ingen officiell klassplacering',values:placementPoints.map(point=>point.class)},sex:{id:'sex',label:'Kön',available:!raceValue.isTeam&&placementPoints.some(point=>finite(point.sex)),message:raceValue.isTeam?`Könsplacering används inte för ${raceValue.participant.plural}`:'Placering saknas',values:placementPoints.map(point=>point.sex)}}};
    }
    // Positive differences always mean A is ahead / gained time. Never interpolate here.
    function headToHeadAnalysis(raceValue,recordA,recordB){
      const raceItem=race(typeof raceValue==='string'?raceValue:raceValue?.key);
      const resolve=value=>record(typeof value==='string'?value:value?.id);
      const aRecord=resolve(recordA),bRecord=resolve(recordB);
      if(!raceItem||!aRecord||!bRecord||aRecord.id===bRecord.id||aRecord.raceKey!==raceItem.key||bRecord.raceKey!==raceItem.key)return null;
      const aProfile=profile(aRecord),bProfile=profile(bRecord);
      const fieldReference=journeyReference('field',raceItem.isTeam?(raceItem.uiLabels.field||'Hela deltagarfältet'):'Hela fältet','#0b6671',journeyCompleteProfiles(raceItem),raceItem);
      const checkpointList=raceItem.analysisCheckpoints.slice(1),finishKey=checkpointList.at(-1)?.key;
      const observation=(p,cp)=>p.record.status==='DNS'||(cp.key===finishKey&&!statusFinished(p.record))?null:p.analysisAnchors.find(anchor=>anchor.checkpoint===cp.key&&anchor.source)||null;
      const placementAt=(p,cp,anchor)=>cp.key===finishKey?(statusFinished(p.record)?number(p.record.overall_place):null):number(anchor?.placeOverall);
      const checkpoints=checkpointList.map((cp,index)=>{
        const a=observation(aProfile,cp),b=observation(bProfile,cp);
        return{index,checkpoint:cp.key,name:checkpointName(raceItem,cp),distance:Number(cp.route_distance_km),elapsedA:a?.elapsedSeconds??null,elapsedB:b?.elapsedSeconds??null,gapSeconds:a&&b?journeyGap(b.elapsedSeconds,a.elapsedSeconds):null,placeA:placementAt(aProfile,cp,a),placeB:placementAt(bProfile,cp,b)};
      });
      const segments=checkpointList.map((cp,index)=>{
        const from=raceItem.analysisCheckpoints[index],to=cp,fromDistance=Number(from.route_distance_km),toDistance=Number(to.route_distance_km);
      ...
      const maxWon=side=>segments.filter(s=>side==='a'?s.segmentDeltaSeconds>0:s.segmentDeltaSeconds<0).sort((a,b)=>Math.abs(b.segmentDeltaSeconds)-Math.abs(a.segmentDeltaSeconds))[0]||null;
      const byGap=sharedCheckpoints.slice().sort((a,b)=>Math.abs(a.gapSeconds)-Math.abs(b.gapSeconds));
      const participant=(item,p)=>({record:item,profile:p,displayName:item.name,summary:{status:item.status,finishSeconds:statusFinished(item)?Number(item.finish_seconds):null,overallPlace:statusFinished(item)?number(item.overall_place):null,classPlace:(!raceItem.isTeam||relayClassMeta(item).ranked)?number(item.class_place):null,percentile:raceItem.isTeam?null:percentile(item,raceItem.records),relayClass:raceItem.isTeam?relayClassMeta(item):null,memberCount:raceItem.isTeam?(team(item)?.team_members?.length??null):null,checkpointCount:checkpoints.filter(cp=>finite(item.id===aRecord.id?cp.elapsedA:cp.elapsedB)).length}});
      return{race:raceItem,a:participant(aRecord,aProfile),b:participant(bRecord,bProfile),checkpoints,sharedCheckpoints,segments,placement:{a:checkpoints.map(cp=>cp.placeA),b:checkpoints.map(cp=>cp.placeB)},fieldReference,fieldPerformance:{a:segments.map(s=>s.performanceA),b:segments.map(s=>s.performanceB)},insights:{finalGapSeconds:statusFinished(aRecord)&&statusFinished(bRecord)?journeyGap(bRecord.finish_seconds,aRecord.finish_seconds):null,leaders,leadChanges,mostTimeWonA:maxWon('a'),mostTimeWonB:maxWon('b'),nearest:byGap[0]||null,largestGap:byGap.at(-1)||null},coverage:{sharedCheckpoints:sharedCheckpoints.length,comparableSegments:segments.filter(s=>finite(s.segmentDeltaSeconds)).length,totalSegments:segments.length}};
    }
    ...
    function segmentStats(recordList){
      ...
      return raceValue.analysisCheckpoints.slice(1).map((checkpoint,index)=>{
        ...
        return{index,checkpoint,name:samples[0]?.name||`${checkpointName(raceValue,raceValue.analysisCheckpoints[index])}–${checkpointName(raceValue,checkpoint)}`,count:samples.length,medianPace:median(paces),q25Pace:quantile(paces,.25),q75Pace:quantile(paces,.75),medianTime:median(times),fastest:Math.min(...times.filter(finite),Infinity),samples};
      });
    }
    ...
    function percentile(value,recordList){
      const item=typeof value==='string'?record(value):value,finishers=recordList.filter(statusFinished);if(!item||!statusFinished(item)||!finishers.some(candidate=>candidate.id===item.id))return null;const slower=finishers.filter(candidate=>Number(candidate.finish_seconds)>Number(item.finish_seconds)).length;return Math.round(slower/finishers.length*100);
    }
    ...
    function personalRaceSummary(value){
      const item=typeof value==='string'?record(value):value,raceValue=race(item?.raceKey),profileValue=item?profile(item):null,status=item?.status||null,finish=statusFinished(item),dns=status==='DNS',referenceRecords=raceValue?.isTeam?relayClassRecords(raceValue.records,item):raceValue?.records||[],relative=!dns?relativeProfile(item,referenceRecords):{strongest:null,weakest:null},relayMeta=raceValue?.isTeam?relayClassMeta(item):null,rankedRelay=Boolean(raceValue?.isTeam&&relayMeta.ranked),fieldPercentile=finish?percentile(item,raceValue.records):null,classPercentile=finish&&rankedRelay?relayClassPercentile(item,raceValue.records):null,segments=profileValue?.segments||[],moves=segments.filter(segment=>finite(segment.placeGain)),gain=moves.slice().sort((a,b)=>b.placeGain-a.placeGain)[0]||null,loss=moves.slice().sort((a,b)=>a.placeGain-b.placeGain)[0]||null,observed=(profileValue?.analysisAnchors||[]).filter(anchor=>anchor.checkpoint!==raceValue?.analysisCheckpoints?.[0]?.key),last=observed.at(-1)||null,total=Math.max(0,(raceValue?.analysisCheckpoints?.length||1)-1),coverage={observed:observed.length,total,complete:Boolean(profileValue?.complete)};
      const referenceLabel=raceValue?.isTeam?(raceValue.uiLabels?.class_reference||'egen klass').toLocaleLowerCase('sv'):'hela fältet';let story=[];if(dns)story=['Ingen registrerad start eller splitberättelse finns.'];else if(finish){story.push(`${item.name} gick i mål på ${Math.floor(item.finish_seconds/3600)}:${String(Math.floor(item.finish_seconds%3600/60)).padStart(2,'0')}:${String(Math.round(item.finish_seconds%60)).padStart(2,'0')}.`);if(relative.strongest)story.push(`${relative.strongest.name} var loppets bästa segment relativt ${referenceLabel}.`);if(gain?.placeGain>0)story.push(`Den största officiella placeringsvinsten var ${gain.placeGain} platser på ${gain.name}.`)}else{story.push(last?`Senaste verkliga analyskontroll var ${last.name} efter ${Math.round(last.elapsedSeconds/60)} minuter.`:'Inga verkliga analyskontroller finns registrerade.');story.push(`Täckning ${coverage.observed} av ${coverage.total} analyskontroller.`)}
  ...
  window.GDataAdapter={create,median,quantile,average,distributionSummary,statusFinished,statusStarter,normalizeClubName,clubKey,normalizeRelayClassName,relayClassMeta:genericClassMeta,MIN_REFERENCE_SIZE};
})();
