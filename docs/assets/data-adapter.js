(function(){
  ...
  function create(data,route,elevation){
    ...
    function headToHeadAnalysis(raceValue,recordA,recordB){
      ...
      const fieldReference=journeyReference('field',raceItem.isRelay?'Alla lag':'Hela fältet','#0b6671',journeyCompleteProfiles(raceItem),raceItem);
      ...
      const checkpoints=checkpointList.map((cp,index)=>{
        ...
        return{index,checkpoint:cp.key,name:cp.name==='Mål'?'Alingsås':cp.name,distance:Number(cp.route_distance_km),elapsedA:a?.elapsedSeconds??null,elapsedB:b?.elapsedSeconds??null,gapSeconds:a&&b?journeyGap(b.elapsedSeconds,a.elapsedSeconds):null,placeA:placementAt(aProfile,cp,a),placeB:placementAt(bProfile,cp,b)};
      });
      ...
      const participant=(item,p)=>({record:item,profile:p,displayName:item.name,summary:{status:item.status,finishSeconds:statusFinished(item)?Number(item.finish_seconds):null,overallPlace:statusFinished(item)?number(item.overall_place):null,classPlace:(!raceItem.isRelay||relayClassMeta(item).ranked)?number(item.class_place):null,percentile:raceItem.isRelay?null:percentile(item,raceItem.records),relayClass:raceItem.isRelay?relayClassMeta(item):null,memberCount:raceItem.isRelay?(team(item)?.team_members?.length??null):null,checkpointCount:checkpoints.filter(cp=>finite(item.id===aRecord.id?cp.elapsedA:cp.elapsedB)).length}});
