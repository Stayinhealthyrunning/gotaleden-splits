(function(){
  ...
  function create(data,courseInput,elevationInput){
    ...
    function headToHeadAnalysis(raceValue,recordA,recordB){
      ...
      const fieldReference=journeyReference('field',raceItem.isTeam?'Alla lag':'Hela fältet','#0b6671',journeyCompleteProfiles(raceItem),raceItem);
      const checkpointList=raceItem.analysisCheckpoints.slice(1);
      const observation=(p,cp)=>p.record.status==='DNS'||(cp.key==='alingsas'&&!statusFinished(p.record))?null:p.analysisAnchors.find(anchor=>anchor.checkpoint===cp.key&&anchor.source)||null;
      const placementAt=(p,cp,anchor)=>cp.key==='alingsas'?(statusFinished(p.record)?number(p.record.overall_place):null):number(anchor?.placeOverall);
