(function(){
  'use strict';
  const KEY='gotaleden-favorites-v1';
  ...
  function create({storage,key=KEY}={}){
    ...
    try{const parsed=JSON.parse(target?.getItem(key)||'[]');if(Array.isArray(parsed))ids=[...new Set(parsed.filter(id=>typeof id==='string'&&id.includes(':')))]}catch{}
