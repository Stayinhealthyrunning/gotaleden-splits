(function(){
  ...
  function routeSegment(adapter,from,to,race){
    const route=race&&adapter.course?adapter.course(race).route:adapter.route,start=Math.max(0,Number(from)||0),end=Math.max(start,Number(to)||0),points=[adapter.routePoint(start,race)];
    ...
    points.push(adapter.routePoint(end,race));return points.filter(validLatLng).map(point=>[Number(point[0]),Number(point[1])]);
  }
  ...
  function createLeaflet(container,{adapter,race,mode,segments=[],onSeek=null,onSegmentSelect=null,onSegmentPreview=null,onSegmentRestore=null,layer='standard'}){
    ...
    const map=L.map(elements.mapElement,{zoomControl:true,preferCanvas:!onSegmentSelect,attributionControl:true,keyboard:true}),bounds=L.latLngBounds(coordinates);
    ...
    if(segments.length){for(const segment of segments){const points=routeSegment(adapter,segment.from.distance,segment.to.distance,race),line=L.polyline(points,{color:segment.color||routeColor,weight:6,opacity:segment.neutral?.58:.96,lineCap:'round',interactive:true}).addTo(routeGroup),hit=L.polyline(points,{color:'#fff',weight:22,opacity:.001,lineCap:'round',interactive:true}).addTo(routeGroup);if(hit._path){hit._path.dataset.mapSegment=segment.index;hit._path.setAttribute('aria-label',`Välj delsträcka ${segment.name}`);hit._path.setAttribute('role','button');hit._path.setAttribute('tabindex','0')}line.bindTooltip(`${esc(segment.name)} · ${Number(segment.distance).toFixed(1)} km · ${segment.paceLabel||'tempo saknas'}${Number.isFinite(segment.placeGain)?` · ${segment.placeGain>0?'+':''}${segment.placeGain} platser`:''}`,{sticky:true,className:'segment-map-tooltip'});const select=event=>{L.DomEvent.stopPropagation(event);if(onSegmentSelect)onSegmentSelect(segment.index,segment);else if(onSeek)onSeek((segment.from.distance+segment.to.distance)/2,{source:'segment',segment})};[line,hit].forEach(layerItem=>{layerItem.on('click',select);if(onSegmentPreview)layerItem.on('mouseover',()=>onSegmentPreview(segment.index,segment));if(onSegmentRestore)layerItem.on('mouseout',onSegmentRestore)});hit._path?.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();select(event)}})}}else L.polyline(coordinates,{color:routeColor,weight:5,opacity:.97,lineCap:'round'}).addTo(routeGroup);
    for(const checkpoint of race.checkpoints){
      const point=adapter.routePoint(checkpoint.route_distance_km,race);if(!validLatLng(point))continue;
      const finish=checkpoint===race.checkpoints.at(-1),start=checkpoint===race.checkpoints[0],timingOnly=checkpoint.timing_only===true,marker=L.circleMarker([point[0],point[1]],{radius:finish||start?8:timingOnly?3.5:5,color:timingOnly?'#66756f':routeColor,weight:timingOnly?1.5:2,fillColor:finish?'#db7b3b':start?'#f1cc6e':checkpoint.is_relay_exchange?'#d8edcd':timingOnly?'#e8eeeb':'#fffefa',fillOpacity:1}).addTo(routeGroup);
  ...
  function createFallback(container,{adapter,race,mode,segments=[],onSegmentSelect=null,onSegmentPreview=null,onSegmentRestore=null}){
    ...
    const segmentPaths=segments.map(segment=>{const points=routeSegment(adapter,segment.from.distance,segment.to.distance,race),d=points.map((point,index)=>{const value=project(point);return`${index?'L':'M'}${value[0].toFixed(1)} ${value[1].toFixed(1)}`}).join(' ');return`<path class="fallback-segment-hit" data-map-segment="${esc(segment.index)}" d="${d}" tabindex="0" role="button" aria-label="Välj delsträcka ${esc(segment.name)}"/>`}).join('');container.innerHTML=`<div class="route-map fallback-map" data-map-engine="fallback"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Förenklad reservkarta"><path class="route-shadow" d="${path}"/><path class="route-line-map" d="${path}"/>${segmentPaths}<g data-fallback-highlight></g><g data-fallback-runners></g></svg><div class="map-engine-status">Interaktiv kartbakgrund saknas. Förenklad banvy används.</div></div>`;const runners=container.querySelector('[data-fallback-runners]'),highlight=container.querySelector('[data-fallback-highlight]');let destroyed=false;
