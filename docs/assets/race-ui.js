(function(){
  ...
  function event(data){
    ...
    const key=clean(source.event_key)||'event',storageNamespace=namespace(source.storage_namespace||key),customNamespace=namespace(source.custom_event_namespace||storageNamespace),presentation=source.presentation||{},model={key,displayName:clean(source.name)||'Running event',productTitle:clean(source.product_title)||clean(source.name)||'Race analysis',shortName:clean(source.short_name)||clean(source.name)||'Event',officialUrl:source.official_site_url||'',storageNamespace,legacyStorageNamespaces:(source.legacy_storage_namespaces||[]).map(namespace).filter(value=>value!==storageNamespace),customNamespace,presentation,helpContent:source.help_content||{},storageKey:suffix=>`${storageNamespace}:${suffix}`,legacyStorageKeys:suffix=>(source.legacy_storage_namespaces||[]).map(value=>`${namespace(value)}-${suffix}`),eventName:name=>`${customNamespace}:${name}`};
