(function(){
  function create(canvas,route){
    ...
    function estimatedDistance(r,time){const values=r.splits;if(!values.length)return null;if(time<=0)return r.start_distance_km;if(time>=values[values.length-1].elapsed_seconds)return values[values.length-1].distance;let prev={elapsed_seconds:0,distance:r.start_distance_km},next=values[0];for(const value of values){if(value.elapsed_seconds>=time){next=value;break}prev=value}const share=(time-prev.elapsed_seconds)/(next.elapsed_seconds-prev.elapsed_seconds||1);return prev.distance+(next.distance-prev.distance)*share}
