...
test('acceptance panels, groups, percentiles and profile layout use the retained analyses',async({page})=>{
  ...
  await page.locator('[data-target="segments"]').click();const percentile=page.locator('#percentile-ladder .percentile-timeline');await expect(percentile).toBeVisible();await expect(percentile.locator('.percentile-marker')).toHaveCount(10);expect(Number(await percentile.getAttribute('data-time-max'))).toBeGreaterThan(Number(await percentile.getAttribute('data-time-min')));
...
test('course difficulty keeps map, elevation, distribution, KPI and table on one selected segment',async({page})=>{
  ...
  const elevationHit=course.locator('[data-course-elevation] [data-course-segment="2"]');expect(await elevationHit.evaluate(element=>Boolean(element.onclick))).toBe(true);await elevationHit.click();await expectSelected(2);const mapVisual=await course.locator('[data-course-map] .route-range-highlight').evaluate(element=>({strokeWidth:parseFloat(getComputedStyle(element).strokeWidth),paneZ:Number(getComputedStyle(element.closest('.route-segment-highlight-pane')).zIndex),visible:element.getBoundingClientRect().width>0&&element.getBoundingClientRect().height>0}));expect(mapVisual.strokeWidth).toBeGreaterThanOrEqual(11);expect(mapVisual.paneZ).toBeGreaterThan(400);expect(mapVisual.visible).toBe(true);
