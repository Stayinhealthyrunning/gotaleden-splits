...
test('an alternate four-family event and two-person Duo use the same generic runtime',async({page})=>{
  ...
  await expect(page.getByRole('heading',{name:'Coast Trail Explorer'})).toBeVisible();await expect(page.locator('#race-switch [role="tab"]')).toHaveCount(4);await expect(page.locator('#race-subtitle')).toContainText('62 km');await expect(page.locator('.hero__route')).toContainText('Salt Marsh');await expect(page.locator('.hero__route')).toContainText('Harbor Light');await expect(page.locator('[data-target="goal-pace"]')).toBeVisible();await expect(page.locator('#sex-filter-field')).toBeVisible();
  ...
  await page.getByRole('tab',{name:/Duo Long/}).click();await expect(page.locator('[data-target="runner-lookup"]')).toHaveText('Duos');await expect(page.locator('[data-target="goal-pace"]')).toBeHidden();await expect(page.locator('#sex-filter-field')).toBeHidden();await expect(page.locator('#club-filter-field')).toBeHidden();
  await page.locator('#runner-search').fill('Sea Duo 1');await page.locator('#runner-suggestions [data-record-id]').first().click();await expect(page.locator('#detail-dialog')).toBeVisible();await expect(page.locator('.detail-members span')).toHaveCount(2);await expect(page.locator('#detail-replay [data-map-engine]')).toBeVisible();await expect(page.locator('#detail-dialog')).toContainText('DUO ANALYSIS');await page.locator('#detail-dialog .dialog-close').click();
  ...
  const visibleText=await page.locator('body').innerText();for(const forbidden of ['Gotaleden','Göteborg','Floda','Alingsås'])expect(visibleText).not.toContain(forbidden);expect(await page.evaluate(()=>document.documentElement.scrollWidth>document.documentElement.clientWidth)).toBe(false);expect(errors).toEqual([]);
});
