...
""")
    ...
    def test_relay_course_copy_uses_teams(self):
        self.assertIn("race.isTeam?'Alla lag':'Hela fältet'",self.course)
        self.assertIn("race.isTeam?'Hela loppets kompletta lag i mål':'Hela loppets kompletta målgångare'",self.course)
        self.assertIn("race.isTeam?'LAGENS':'FÄLTETS'",self.course)
