...
class DistributionBandChartTests(unittest.TestCase):
    ...
    def test_band_median_outer_tooltip_and_click_targets(self):
        self.run_node(r"""
...
const html=window.GCharts.distributionBand([{id:'F',name:'Kvinnor',color:'#db2777',n:20,segments:[d(20,360,330,390,310,420),d(7,370)]}],['A','B'],{format:v=>Math.round(v)+'s',unitLabel:' /km'});
for(const token of ['class="distribution-band"','class="plot-line distribution-median"','data-series="F"','data-course-segment="0"','Q25–Q75: 330s–390s /km','Q10–Q90: 310s–420s /km','n=7 · Litet underlag'])if(!html.includes(token))throw new Error(token);
